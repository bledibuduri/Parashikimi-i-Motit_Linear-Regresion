...
y = data['Air_Pressure']  # Target column (Lagështira)
...
all_data = pd.concat([data[['Timestamp', 'Air_Pressure']], predicted_2024], ignore_index=True)
...
fig = px.line(all_data, x='Timestamp', y='Air_Pressure', title="Air Preasure From 2018-2024")
...
plt.title("Air Preasure From 2018-2024")
sns.lineplot(x=data['Timestamp'], y=data['Air_Pressure'], marker='o', color='red')
plt.xlabel("Year")
plt.ylabel("Air Preasure")
...
avg_monthly_airpreasure = pd.DataFrame(all_data.groupby([all_data["Timestamp"].dt.month])["Air_Pressure"].mean())
avg_monthly_airpreasure.reset_index(inplace=True)

plt.figure(figsize=(20,10))
plt.title("Monthly Average Air Preasure From 2018-2024")
sns.lineplot(x=avg_monthly_airpreasure['Timestamp'],y=avg_monthly_airpreasure['Air_Pressure'])
plt.xlabel("Month")
#plt.show()

plt.figure(figsize=(20,10))
plt.title("Presioni i Ajrit - Mesatarja Mujore Nga 2018-2024")
sns.barplot(x=avg_monthly_airpreasure['Timestamp'],y=avg_monthly_airpreasure['Air_Pressure'], palette="mako")
plt.xlabel("Month")
plt.show()
